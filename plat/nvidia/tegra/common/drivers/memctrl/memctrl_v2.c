				 ...
				 uint32_t non_overlap_area_size)
{
	...
	tegra_zeromem16(non_overlap_area_start, non_overlap_area_size);

	(void)mmap_remove_dynamic_region(non_overlap_area_start,
		non_overlap_area_size);
}

static void tegra_clear_videomem_nonoverlap(uint64_t phys_base,
					    uint32_t size_in_bytes)
{
	uint64_t vmem_end_old = video_mem_base + (video_mem_size_mb << 20);
